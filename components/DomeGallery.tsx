...
function buildItems(pool: any[], seg: number) {
  ...
  const normalizedImages = pool.map(image => {
    ...
    return { src: image.src || image.image || '', alt: image.alt || image.title || '' };
  });
...
export default function DomeGallery({
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    if (wantsResize) {
      const onFirstEnd = (ev: TransitionEvent) => {
        ...
        void (overlay as any).offsetWidth;
  ...
  const cssStyles = `
    ...
    .item__image {
      ...
      -webkit-backface-visibility: hidden; transition: transform 300ms;
      ...
      >
        ...
        >
              ...
              {items.map((it, i) => (
                ...
                >
                  <div
                    ...
                    style={{ inset: '10px', borderRadius: `var(--tile-radius, ${imageBorderRadius})`, backfaceVisibility: 'hidden' } as any}
                  >
                    <img
                      src={it.src} draggable={false} alt={it.alt}
                      className="w-full h-full object-cover pointer-events-none"
                      style={{ backfaceVisibility: 'hidden', filter: `var(--image-filter, ${grayscale ? 'grayscale(1)' : 'none'})` } as any}
                    />
                  </div>
                </div>
              ))}
          ...
          <div ref={viewerRef} className="absolute inset-0 z-20 pointer-events-none flex items-center justify-center"
            style={{ padding: 'var(--viewer-pad)' } as any}
          >
